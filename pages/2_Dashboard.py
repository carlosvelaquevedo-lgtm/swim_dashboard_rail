...
if not st.session_state.get("paid", False) and not IS_DEV:
    # Try to recover payment state from query params (handles mobile reconnect)
    verified_session = st.query_params.get("verified")
    if verified_session:
        if verify_stripe_payment(verified_session):
